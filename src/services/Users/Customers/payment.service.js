...
const createOrder = async ({order, user_id}) => {
  ...
  // Embed data for mobile redirect (sử dụng deeplink để chuyển hướng người dùng trở lại app di động sau khi thanh toán)
  const embed_data = {
  ...
  const configOrder = {
    ...
    amount: order.price,
    callback_url: `${process.env.URL_SYSTEM}/callback`, // API server để xử lý callback
    description : `
Thanh toán cho đơn hàng #${
    
    order.listCartItem.map(item => `
...
`,
  ...
  try {
    const result = await axios.post(config.endpoint, null, { params: configOrder });
    return result.data;
  } catch (error) {
